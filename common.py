...
def pine_fvg_zones(levels, dampen):
    """สร้างโซน FVG / Imbalance ต่อท้าย PineScript
    levels : list ของ (name, value) — ใช้ตรวจว่าโซนไหน 'ทับเส้นสำคัญ' (ไฮไลต์พิเศษ)
    dampen : True=โหมดหน่วง • False=โหมดเร่ง (ใส่ในข้อความแจ้งเตือน)
    นิยาม FVG (3 แท่ง): ขาขึ้น = low > high[2] • ขาลง = high < low[2]
    - กล่องยืดตามเวลา • ลบเมื่อถูกถมแล้ว (mitigated)
    - โซนที่ทับเส้นสำคัญ = สีทอง + ป้ายชื่อเส้น (จุดพิจารณาเข้าที่ชัดที่สุด)
    - ประเมินบน barstate.isconfirmed เท่านั้น (กัน repaint)"""
    if not levels:
        return []
    mode = "false" if dampen else "true"
    ...
    return [
        ...
        'fvgLvlOnly = input.bool(false, "แสดงเฉพาะโซนที่ทับเส้นสำคัญ", group=grpF)',
        'fvgAlertOn = input.bool(true,  "แจ้งเตือนเมื่อราคาเข้าโซน", group=grpF)',
        'lvlTolPct  = input.float(0.15, "ระยะนับว่าทับเส้น %", minval=0.01, step=0.05, group=grpF) / 100',
        f"fvgAccel   = {mode}",
        f"var fLvlP  = array.from({pv})",
        f"var fLvlN  = array.from({pn})",
        "",
        "var box[]    fBox  = array.new_box()",
        "var float[]  fTop  = array.new_float()",
        "var float[]  fBot  = array.new_float()",
        "var bool[]   fBull = array.new_bool()",
        "var bool[]   fAtL  = array.new_bool()",
        "var string[] fName = array.new_string()",
        "var bool[]   fIn   = array.new_bool()",
        "",
        "// หาว่าโซนนี้ทับเส้นสำคัญไหน (คืนชื่อเส้น, ว่าง = ไม่ทับ)",
        "f_lvlHit(_t, _b) =>",
        '    string _nm = ""',
        "    if array.size(fLvlP) > 0",
        "        for i = 0 to array.size(fLvlP) - 1",
        "            _v = array.get(fLvlP, i)",
        "            if _v >= _b - _v * lvlTolPct and _v <= _t + _v * lvlTolPct",
        "                _nm := array.get(fLvlN, i)",
        "                break",
        "    _nm",
        "",
        "// ----- ตรวจจับ FVG ใหม่ (แท่งปิดแล้วเท่านั้น) -----",
        ...
        "    _t  = newBull ? low : low[2]",
        "    _b  = newBull ? high[2] : high",
        "    _nm = f_lvlHit(_t, _b)",
        '    _atL = _nm != ""',
        "    if (not fvgLvlOnly) or _atL",
        "        _bg = _atL ? color.new(color.yellow, 80) : (newBull ? color.new(color.green, 90) : color.new(color.red, 90))",
        "        _bc = _atL ? color.new(color.yellow, 30) : color.new(color.gray, 60)",
        "        _bx = box.new(bar_index - 2, _t, bar_index + 3, _b, border_color=_bc, bgcolor=_bg)",
        "        if _atL",
        '            box.set_text(_bx, "⭐ FVG @ " + _nm)',
        "            box.set_text_color(_bx, color.white)",
        "            box.set_text_size(_bx, size.tiny)",
        "            box.set_text_halign(_bx, text.align_left)",
        "        array.push(fBox, _bx)",
        "        array.push(fTop, _t)",
        "        array.push(fBot, _b)",
        "        array.push(fBull, newBull)",
        "        array.push(fAtL, _atL)",
        "        array.push(fName, _nm)",
        ...
        "            array.shift(fIn)",
        "",
        "// ----- ยืดกล่อง • แจ้งเตือนเมื่อราคาเข้าโซน • ลบเมื่อถูกถม -----",
        'fvgModeTh = fvgAccel ? "เร่ง (มักทะลุ)" : "หน่วง (มักเด้ง)"',
        "if barstate.isconfirmed and array.size(fBox) > 0",
        "    for i = array.size(fBox) - 1 to 0",
        "        _t = array.get(fTop, i)",
        "        _b = array.get(fBot, i)",
        "        _bx = array.get(fBox, i)",
        "        box.set_right(_bx, bar_index + 3)",
        "        _inside = low <= _t and high >= _b",
        "        if _inside and not array.get(fIn, i)",
        "            array.set(fIn, i, true)",
        "            if fvgAlertOn",
        '                _dir = array.get(fBull, i) ? "ขาขึ้น" : "ขาลง"',
        '                _tag = array.get(fAtL, i) ? " ⭐ ที่เส้น " + array.get(fName, i) : ""',
        '                alert("🎯 ราคาเข้าโซน FVG " + _dir + _tag + " • โหมด " + fvgModeTh, alert.freq_once_per_bar_close)',
        "        _mit = array.get(fBull, i) ? close < _b : close > _t",
        "        if _mit",
        "            box.delete(_bx)",
        "            array.remove(fBox, i)",
        "            array.remove(fTop, i)",
